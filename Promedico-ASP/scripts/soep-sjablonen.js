(function() {
    ...
    const templates = {
        ...
        'UWI': {
            submenu: {
                ...
                'Kweek: start AB': {
                    P: 'Urinekweek toont groei van bacterie, gevoelig voor ingezette antibiotica. Kuur afmaken. Terugkomen bij aanhoudende klachten.'
                },
                'Kweek: switch AB': {
                    P: 'Urinekweek toont resistentie voor ingezette antibiotica. Nieuw recept gestuurd naar apotheek. Eerste kuur stoppen, nieuwe kuur starten en afmaken.'
                },
                'Kweek: mengbeeld': {
    ...
    // Gedeelde ICPC-setter: werkt voor alle codes ongeacht of ze in de popup-lijst staan.
    // Vult het ICPC-veld in, opent de popup, en voert dan de save()-logica direct
    // uit in het parent frame (omzeilt de crash die optreedt als de code niet in
    // de gefilterde lijst staat). Optionele callback wordt na sluiten aangeroepen.
    function setICPC(icpcCode, callback) {
        ...
        setTimeout(() => {
            ...
            const pollInterval = setInterval(() => {
                ...
                if (!icpcFrame) {
                    if (attempts > 20) { clearInterval(pollInterval); if (callback) callback(); }
                    return;
                }
                try {
                    if (icpcFrame.document.readyState !== 'complete') return;
                    clearInterval(pollInterval);

                    // Voer de save()-logica direct uit in het parent frame
                    // zodat we niet afhankelijk zijn van getElementById(icpcCode) in de popup
                    const par = icpcFrame.contentWindow
                        ? icpcFrame.contentWindow.parent
                        : icpcFrame.parent;
                    const doc = par.document;

                    if (doc.contactform && doc.contactform['contactForm.icpcCode']) {
                        doc.contactform['contactForm.icpcCode'].value = icpcCode;
                    }

                    // E-veld alleen overschrijven als nog leeg
                    const eVeld = doc.contactform && doc.contactform['contactForm.regelE'];
                    if (eVeld && eVeld.value.trim().length < 1) {
                        eVeld.value = icpcCode;
                    }

                    // Episode ICPC als nog leeg
                    const epIcpc = doc.getElementById('icpcCodeEpisode');
                    if (epIcpc && (!epIcpc.value || epIcpc.value === '')) {
                        epIcpc.value = icpcCode;
                    }

                    if (typeof par.updateERegel === 'function') par.updateERegel();
                    if (typeof par.closeIFrame === 'function') {
                        par.closeIFrame(doc.contactform && doc.contactform['contactForm.icpcCode']
                            ? 'zoek' : 'zoekicpc');
                    }
    ...
    function applyTemplate(template) {
        ...
        if (template.icpc) {
            setICPC(template.icpc);
        }
    }

    // CRP action
    function executeCRPAction() {
        if (_settings.crpPoct) {
            // POCT Connect: kopieer BSN en open website
            ...
            // Handmatige invoer: zet tekst in O-veld
            insertText('contactForm.regelO', 'CRP geprikt, uitslag: … mg/L');
        }
    }
    ...
    // BVO action — identiek aan applyTemplate/U71, maar met X49 en BVO-teksten
    function executeBVOAction() {
        const koerier = _settings.bvoKoerier;
        ...
        insertText('contactForm.regelP', koerier
            ? 'Uitstrijkje wordt opgehaald door koerier, zorgdomein verwijzing gemaakt. Patient krijgt uitslag thuis.'
            : 'Uitstrijkje afgenomen, zorgdomein verwijzing gemaakt. Patient krijgt uitslag thuis.');
    ...
    function createMenu() {
        ...
        // Niet tonen bij e-consulten: die hebben geen O-veld (contactForm.regelO)
        // E-consult heeft Reactie(P) ipv SOEP-structuur
        const oField = document.getElementById('contactForm.regelO');
        if (!oField) {
            return; // e-consult context — e-consult-templates.js handelt dit af
        }
