...
async function loadSettings() {
  ...
  // Toon actief regio-profiel
  const regioLabel = document.getElementById('regioLabel');
  if (regioLabel && typeof PROFILES !== 'undefined') {
    const activeId = settings.activeProfile || null;
    const profile = PROFILES.find(p => p.id === activeId);
    regioLabel.textContent = profile ? profile.name : 'Niet ingesteld';
  }
}
...
function setupEventListeners() {
  ...
  const btnWisselRegio = document.getElementById('btnWisselRegio');
  if (btnWisselRegio) {
    btnWisselRegio.addEventListener('click', () => {
