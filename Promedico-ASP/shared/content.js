...
(function() {
  ...
  // ── Bridge: luistert naar postMessage van page context scripts ──────────────
  // Scripts draaien in page context en hebben geen directe toegang tot
  // extension storage of runtime. Ze sturen berichten via window.postMessage,
  // wij voeren de echte aanroep uit en sturen het resultaat terug.
  window.addEventListener('message', function(event) {
    ...
    else if (method === 'actie.bellen')         promise = _api.runtime.sendMessage({ type: 'belNummer', nummer: args[0] });
    else return;
    ...
    injectPageContextShim(() => {
      config.scripts.forEach(script => {
        ...
        if (script.id === 'zorgdomeinQuickMenu') {
          const menuFile = settings.activeMenuFile || 'zorgdomein-menu-data.js';
